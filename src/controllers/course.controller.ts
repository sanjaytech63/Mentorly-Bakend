...
export const getCourses = asyncHandler(async (req: Request, res: Response) => {
  try {
    ...
    return res.status(200).json(new ApiResponse(200, responseData, 'Courses fetched successfully'));
  } catch (error: any) {
    console.error('Error fetching courses:', error);
    return res.status(500).json(new ApiError(500, 'Failed to fetch courses', error.message));
  }
});
...
export const getCourseById = asyncHandler(async (req: Request, res: Response) => {
  try {
    ...
    return res.status(200).json(
      // ✅ FIXED: Changed from 201 to 200 for GET
      new ApiResponse(200, responseData, 'Course fetched successfully')
    );
  } catch (error: any) {
    console.error('Error fetching course:', error);
    return res.status(500).json(new ApiError(500, 'Failed to fetch course', error.message));
  }
});
...
export const createCourse = asyncHandler(async (req: Request, res: Response) => {
  try {
    ...
    const missingFields = requiredFields.filter(
      field => !courseData[field as keyof typeof courseData]
    );
    ...
    return res.status(500).json(new ApiError(500, 'Failed to create course', error.message));
  }
});
...
export const updateCourse = asyncHandler(async (req: Request, res: Response) => {
  try {
    ...
    if (req.body.discountedPrice !== undefined) {
      updateData.discountedPrice = req.body.discountedPrice
    ...
    return res.status(500).json(new ApiError(500, 'Failed to update course', error.message));
  }
});
/**
...
export const deleteCourse = asyncHandler(async (req: Request, res: Response) => {
  try {
    ...
    return res.status(201).json(new ApiResponse(201, null, 'Course deleted successfully'));
  } catch (error: any) {
...
export const getCategories = asyncHandler(async (req: Request, res: Response) => {
  try {
    ...
    const allCoursesCount = includeCount
      ? await CourseModel.countDocuments({ isActive: true })
      : null;

    const responseData = {
      categories: [{ name: 'All Courses', count: allCoursesCount }, ...categories],
    };

    return res
      .status(201)
      .json(new ApiResponse(201, responseData, 'Categories fetched successfully'));
  } catch (error: any) {
    console.error('Error fetching categories:', error);
    return res.status(500).json(new ApiError(500, 'Failed to fetch categories'));
  }
});
...
export const getFeaturedCourses = asyncHandler(async (req: Request, res: Response) => {
  try {
    ...
    return res
      .status(201)
      .json(new ApiResponse(201, courses, 'Featured courses fetched successfully'));
  } catch (error: any) {
    console.error('Error fetching featured courses:', error);
    return res.status(500).json(new ApiError(500, 'Failed to fetch featured courses'));
  }
});
...
export const getDiscountedCourses = asyncHandler(async (req: Request, res: Response) => {
  try {
    ...
    return res
      .status(201)
      .json(new ApiResponse(201, courses, 'Discounted courses fetched successfully'));
  } catch (error: any) {
    console.error('Error fetching discounted courses:', error);
    return res.status(500).json(new ApiError(500, 'Failed to fetch discounted courses'));
  }
});
...
export const getCourseStats = asyncHandler(async (req: Request, res: Response) => {
  try {
    ...
    return res
      .status(201)
      .json(new ApiResponse(201, responseData, 'Course statistics fetched successfully'));
  } catch (error: any) {
    console.error('Error fetching course statistics:', error);
    return res.status(500).json(new ApiError(500, 'Failed to fetch course statistics'));
  }
});
  ...
  try {
    ...
    if (minDiscountPercentage !== undefined || maxDiscountPercentage !== undefined) {
      ...
      if (minDiscountPercentage !== undefined)
        filter.discountPercentage.$gte = parseFloat(minDiscountPercentage as string);
      if (maxDiscountPercentage !== undefined)
        filter.discountPercentage.$lte = parseFloat(maxDiscountPercentage as string);
    }
    ...
    const [courses, total, aggregationStats] = await Promise.all([
      ...
      CourseModel.find(filter).sort(sort).skip(skip).limit(limitNum).lean(),
    ...
    return res.status(200).json(new ApiResponse(200, responseData, 'Courses listed successfully'));
  } catch (error: any) {
    console.error('Error listing courses:', error);
    return res.status(500).json(new ApiError(500, 'Failed to list courses', error.message));
  }
});
...
export const quickSearchCourses = asyncHandler(async (req: Request, res: Response) => {
  try {
    ...
    return res
      .status(200)
      .json(new ApiResponse(200, courses, 'Quick search completed successfully'));
  } catch (error: any) {
    console.error('Error in quick search:', error);
    return res.status(500).json(new ApiError(500, 'Failed to perform quick search'));
  }
});
...
export const getSimilarCourses = asyncHandler(async (req: Request, res: Response) => {
  try {
    ...
    return res
      .status(200)
      .json(new ApiResponse(200, similarCourses, 'Similar courses fetched successfully'));
  } catch (error: any) {
    console.error('Error fetching similar courses:', error);
    return res.status(500).json(new ApiError(500, 'Failed to fetch similar courses'));
  }
});
